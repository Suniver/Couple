# coding = gbk
# module name : H5extractRodDeckFile.py
# 这个程序用来帮助将体积较大的Deck.out文件分割成一系列小的xxx.out文件，每个文件都只存储某一个时间步下某个棒的某个面位的所有信息
# 这个程序会在当前脚本的执行目录下新建一系列文件夹，文件夹的名字是小文件存储的时间信息。所有属于同一模拟时间的小文件都会被存储在对应文件夹下
import re
import os

def extractRodDeckFile(DeckFile):
    ...
    RodNumber       = 0             #变量，存储当前小文件中的棒位信息
    SurfaceNumber   = 0             #变量，存储当前小文件中的面位信息
    
    CacheFileName = 'Rod.cache'     #变量：缓存文件的文件名
    ...
    for line in DeckFile:           #对于大文件中的每一行：
        ...
        if re.findall('nuclear fuel rod no.',line) != []:           #首行的判断标准：包含字符串'nuclear fuel rod no.'
            ...
            RodNumber = int(re.findall('nuclear fuel rod no. *(\d+)',line)[0])          #从首行中提取棒位信息
            SimulationTime = float(re.findall('simulation time = *(\d*.\d+)',line)[0])  #从首行中提取时间信息
            
        elif afterStartLine == True and StarLineCount == 2:     #末行的判断标准：读过了起始行，并且读到了第二个星号行
            ...
            if readingStartLine == True:
                SurfaceNumber = int(re.findall('surface no. *(\d+)',line)[0]) #在首行的后一行中提取面位信息
        ...
        if readingStartLine:                        #如果读到了首行
            ...
            newFileName = 'deck.Time%s.rod%s.Surface%s.out'%(SimulationTime, RodNumber, SurfaceNumber)
            newDir = os.path.join(curDir,'%s'%SimulationTime)
            
            if not os.path.isdir(newDir): # check if the folder exists
                os.mkdir(newDir)
            
            os.rename(os.path.join(curDir,CacheFileName),os.path.join(newDir,newFileName)) # move the file by rename
            
            print 'One single rod deck file of simulation time %s, rod number %s, surface number %s has been splited successfully'%(SimulationTime, RodNumber, SurfaceNumber)
